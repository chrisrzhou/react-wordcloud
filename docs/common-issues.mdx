...
The React component will re-render (i.e. growing from the center) if the `size`, `callbacks`, `options` props are updated unintentionally.  Note that `react-wordcloud` is un-opinionated about this rendering behavior since this is the default React behavior (i.e. prop changes re-render the component).
...
<Playground>
  {() => {
    ...
    const memoizedSize = useMemo(() => [600, 300], []);  // memoized. You can also just define the size variable itself outside of the componenet
    return (
      <div>
...
- Large `words` data is provided.
- High `maxWords` prop is applied.
- Small `size` prop or parent container size vs large `fontSizes`.
