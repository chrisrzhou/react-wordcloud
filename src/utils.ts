...
export function choose<T = number | string>(array: T[], randomGenerator: ()=> number): T {
...
export function rotate(rotations: number, rotationAngles: MinMaxPair, randomGenerator: ()=> number): number {
  if (rotations < 1) {
    return 0;
  }
  let angles: number[] = [];
  ...
  return choose(angles, randomGenerator);
}
